...
export const usePongGameLogic = (): returnType => {
    ...
    const drawGame = useCallback((gameState: IGameState) => {
        ...
        ctx.current.fillText(gameState.leftPaddle.score + " | " + gameState.rightPaddle.score, GAME_WIDTH / 2, 80);
        // Ball
        ctx.current.fillStyle = '#689B8A';
        ctx.current.beginPath();
        ctx.current.arc(gameState.ball.x, gameState.ball.y, BALL_RADIUS, 0, 2 * Math.PI);
        ctx.current.fill();
        // left paddle
        ctx.current.fillStyle = '#280A3E';
        ctx.current.fillRect(0, gameState.leftPaddle.y, PADDLE_WIDTH, PADDLE_HEIGHT);
        //right paddle
        ctx.current.fillRect(GAME_WIDTH - PADDLE_WIDTH, gameState.rightPaddle.y, PADDLE_WIDTH, PADDLE_HEIGHT);
        if (gameState.status === 'ended') {
            ctx.current.font = "48px serif";
            ctx.current.textAlign = "center";
            ctx.current.strokeText(gameState.winner === playerRole.current ? "You Won": "You Lost", GAME_WIDTH / 2, GAME_HEIGHT / 2);
            return;
        }
    }, [clearCanvas]);
