...
const NotificationCenter = ({ onClose }: Notification_props) => {
  ...
  const fetchNotifications = async () => {
    try {
      const res = await fetch(`https://localhost:8080/api/chat/notifications/${user.id}`);
    ...
    >
      <div className='max-w-2xl mx-auto'>
        ...
        {/* <div className="flex justify-end mb-4">
          <button
            onClick={() => setIsOpen(!iexport const markAllNotificationsAsRead_game = async (userId: number) => {
...
// };
sOpen)}
            className="relative bg-slate-800 hover:bg-slate-700 text-white p-3 rounded-full transition-all duration-200 shadow-lg"
          >
            <Bell size={24} />
            {totalCount > 0 && (
