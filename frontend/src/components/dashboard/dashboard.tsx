...
export default function DashboardClient({ initialGames }: { initialGames: game[] }) {
    const [allGames, setAllGames] = useState<game[]>(initialGames)
    const [visibleGames, setVisibleGames] = useState<game[]>(initialGames.slice(0, 15))
    const [moreGames, setMoreGames] = useState(initialGames.length > 15 ? 1 : 0)
    const loadRef = useRef<HTMLDivElement>(null)
    const currentOffset = useRef(15)
    const limit = 15
    ...
    const loadMore = useCallback(() => {
        ...
        if (nextOffset >= allGames.length) {
            setMoreGames(0)
        }
    }, [allGames]);
    useEffect(() => {
        currentOffset.current = 15
        setVisibleGames(allGames.slice(0, 15))
        setMoreGames(allGames.length > 15 ? 1 : 0)
    }, [allGames])
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            (entries) => {
                if (entries[0].isIntersecting && moreGames) {
                    loadMore();
                }
            },
            { threshold: 0.1 }
