...
until curl -s -u elastic:${ELASTIC_PASSWORD} --cacert config/certs/ca/ca.crt https://kibana:5601/kibana/api/status | grep -q '"level":"available"'; do
  sleep 5
done


curl -s -X POST "https://kibana:5601/kibana/api/saved_objects/_import" \
  -H "kbn-xsrf: true" \
